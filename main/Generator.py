...
class Generator(nn.Module):
    ...
    def forward(self,x):
        ...
        u2 = self.u2(cat([u1, d7], 1))
        u3 = self.u3(cat([u2, d6], 1))
        u4 = self.u4(cat([u3, d5], 1))
        u5 = self.u5(cat([u4, d4], 1))
        u6 = self.u6(cat([u5, d3], 1))
        u7 = self.u7(cat([u6, d2], 1))
        
        return self.final_up(cat([u7, d1], 1))